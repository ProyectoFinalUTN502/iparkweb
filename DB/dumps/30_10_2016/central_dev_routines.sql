...
--
-- Temporary table structure for view `vw_historic`
--

DROP TABLE IF EXISTS `vw_historic`;
/*!50001 DROP VIEW IF EXISTS `vw_historic`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `vw_historic` (
  `id` tinyint NOT NULL,
  `date` tinyint NOT NULL,
  `vehicle` tinyint NOT NULL,
  `parkinglot` tinyint NOT NULL,
...
--
-- Final view structure for view `vw_historic`
--

/*!50001 DROP TABLE IF EXISTS `vw_historic`*/;
/*!50001 DROP VIEW IF EXISTS `vw_historic`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_historic` AS select `vp`.`id` AS `id`,`vp`.`creationDate` AS `date`,`v`.`name` AS `vehicle`,`p`.`name` AS `parkinglot`,concat(`p`.`address`,', ',`c`.`description`) AS `address`,`p`.`latMap` AS `lat`,`p`.`longMap` AS `lng`,`lp`.`xPoint` AS `xPoint`,`lp`.`yPoint` AS `yPoint`,`v`.`client_id` AS `client_id` from (((((`vehicle_parking` `vp` left join `vehicle` `v` on((`vp`.`vehicle_id` = `v`.`id`))) left join `layout_position` `lp` on((`vp`.`layout_position_id` = `lp`.`id`))) left join `layout` `l` on((`lp`.`layout_id` = `l`.`id`))) left join `parkinglot` `p` on((`l`.`parkinglot_id` = `p`.`id`))) left join `city` `c` on((`p`.`city_id` = `c`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2016-10-30 22:26:26
